...
//i stored the circles in the circles variable
...
for(var i = 0; i<circles.length; i++){
  circles[i].style.background = random[i];
  circles[i].addEventListener('click',function(){
    var clickedCircle = this.style.background;
    
    if(clickedCircle === pickedColor){
...
function pickColor(){
  ...
  var randomPickedColor = Math.floor(Math.random() * colors.length); 
  return colors[randomPickedColor];
  console.log(colors[randomPickedColor]);
}
